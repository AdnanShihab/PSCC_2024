...
from cost_slvg_func import cost_slvg
...
res_heat_produced_CHP_mwh_2023 = []
res_heat_storage_2023 = []
...
res_heat_produced_CHP_mwh_2024 = []
res_heat_storage_2024 = []
...
res_bess_2025_mwh = []
res_p_curt_2025_mwh = []
res_heat_produced_CHP_mwh_2025 = []
res_heat_storage_2025 = []
...
res_bess_2026_mwh = []
res_p_curt_2026_mwh = []
res_heat_produced_CHP_mwh_2026 = []
res_heat_storage_2026 = []
...
res_p_curt_2027_mwh = []
res_bess_2027_mwh = []
res_heat_produced_CHP_mwh_2027 = []
res_heat_storage_2027 = []
...
res_bess_2028_mwh = []
res_p_curt_2028_mwh = []
res_heat_produced_CHP_mwh_2028 = []
res_heat_storage_2028 = []
...
class MyProblem(ElementwiseProblem):
    ...
    def __init__(self, **kwargs):
        ...
        # *********************************** BESS locations ******************************************
        # bus-bars for the BESS - Y2 2024
        for k in range(309, 310):
            variables[f"x{k:01}"] = Integer(bounds=(2, 14))

        # bus-bars for the BESS - Y3 2025
        for k in range(310, 311):
            variables[f"x{k:01}"] = Integer(bounds=(2, 14))

        # bus-bars for the BESS - Y4 2026
        for k in range(311, 312):
            variables[f"x{k:01}"] = Integer(bounds=(2, 14))

        # bus-bars for the BESS - Y5 2027
        for k in range(312, 313):
            variables[f"x{k:01}"] = Integer(bounds=(2, 14))

        # bus-bars for the BESS - Y6 2028
        for k in range(313, 314):
            variables[f"x{k:01}"] = Integer(bounds=(2, 14))
        ...
        super().__init__(vars=variables, n_obj=30, n_ieq_constr=2, **kwargs)

    def _evaluate(self, x, out, *args, **kwargs):
        x = np.array([x[f"x{k:01}"] for k in range(0, 314)])
        ...
        for y in range(len(num_time_steps)):
            ...
            if y == 0:      # 2023
                ...
                res_heat_produced_CHP_mwh_2023.append(h_chp_mwh)
                print("heat_produced_CHP_mwh_2023 =", h_chp_mwh)
                ...
                if p_balance_mwh <= 0:
                    ...
                    print("p_surplus_mw_4_h2_hp [MW] =", p_surplus_mw)
                    # ------- Outputs --------------
                ...
                if p_surplus_mw >= p_p2g_mw:
                    ...
                    if h2_MWh_day > g_demand_mwh_2023:
                        h2_mwh_balance = np.absolute(h2_MWh_day - g_demand_mwh_2023)
                    ...
                    print("p_surplus_4_hp_bess_mw =", p_surplus_4_hp_bess_mw)
                    x_hp = x[291]
                    x_bess = x[271]

                    # ***************** HEAT PUMP AND HEAT STORAGE model x[291] *********************************
                    if p_surplus_4_hp_bess_mw >= x_hp:
                        ...
                        if heat_production_hp_mwh > 0:
                            ...
                            print("heat_production_hp_mwh_2023 = ", heat_production_hp_mwh)
                        ...
                        # .... AFTER HP ...........
                        p_surplus_after_hp_mw = p_surplus_4_hp_bess_mw - x_hp

                        # ******************** BESS function ******************************************************
                        if p_surplus_after_hp_mw >= x_bess:
                            add_bess = power_flow_.create_bess_init()
                            vm = power_flow_.calc_vm()
                            res_bess_2023_mwh.append(x_bess*4.5)        # charging with max sun hour in Germany
                            res_p_curt_2023_mwh.append(((p_surplus_after_hp_mw - x_bess) * 24))

                        # x[271] = p_surplus_4_hp_bess_mw*4.5
                        ...
                        # res_p_curt_2023_mwh.append((p_surplus_4_hp_bess_mw * 24) - x[271])
                            ...
                            # ***************** Energy export ***************************
                            res_p_curt_2023_mwh.append((p_surplus_after_hp_mw*24))
                    # .... if p surplus is lower that x_HP
                    else:
                        # ******************** BESS function ************************************************
                        if p_surplus_4_hp_bess_mw >= x_bess:
                            add_bess = power_flow_.create_bess_init()
                            vm = power_flow_.calc_vm()
                            res_bess_2023_mwh.append(x_bess * 4.5)  # charging with max sun hour in Germany

                            p_surplus_after_bess_mw = p_surplus_4_hp_bess_mw - x_bess
                            if p_surplus_after_bess_mw > 0:
                                res_p_curt_2023_mwh.append(p_surplus_after_bess_mw * 24)
                        else:
                            # ***************** Energy export ***************************
                            res_p_curt_2023_mwh.append((p_surplus_4_hp_bess_mw * 24))
                    ...
                    if h2_MWh_day > g_demand_mwh_2023:
                        h2_mwh_balance = h2_MWh_day - g_demand_mwh_2023
                        res_gas_storage_2023_mwh.append(h2_mwh_balance)
                        gas_import = 0
                        res_gas_import_4ind_2023_mwh.append(gas_import)
                    else:
                        gas_import = g_demand_mwh_2023 - h2_MWh_day
                        res_gas_import_4ind_2023_mwh.append(gas_import)
                        res_gas_storage_2023_mwh.append(0)
                ...
                # Interconnection of HP and CHP
                if res_hp_2023_mwh[-1] >= heat[1]:
                    res_heat_produced_CHP_mwh_2023[-1] = 0
                    # res_heat_produced_CHP_mwh_2023.append(0)
                    h_surplus = res_hp_2023_mwh[-1] - heat[1]

                    # HEAT STORAGE ************************
                    res_heat_storage_2023.append(h_surplus)
                else:
                    res_hp_2023_mwh[-1] = 0

                    # HEAT STORAGE ************************
                    res_heat_storage_2023.append(0)

                print("res_heat_produced_CHP_mwh_2023", res_heat_produced_CHP_mwh_2023)
                print("res_hp_2023_mwh", res_hp_2023_mwh)
                print("res_heat_storage_2023", res_heat_storage_2023)
                ...
                cost_om_hp = cost.cost_om_2023_hp()

                cost_inv_p2g = cost.cost_inv_2023_p2g()
                cost_om_p2g = cost.cost_om_2023_p2g()

                salvage_calc = cost_slvg(stage=idx_w, year=y, cost_pv_inv=cost_inv_pv, cost_pv_om=cost_om_pv,
                                         cost_bess_inv=cost_inv_bess, cost_bess_om=cost_om_bess,
                                         cost_chp_inv=cost_inv_chp, cost_chp_om=cost_om_chp,
                                         cost_hp_inv=cost_inv_hp, cost_hp_om=cost_om_hp,
                                         cost_p2g_inv=cost_inv_p2g, cost_p2g_om=cost_om_p2g)
                cost_salvage_pv = salvage_calc.cost_slvg_pv_2023()
                cost_salvage_bess = salvage_calc.cost_slvg_bess_2023()
                cost_salvage_chp = salvage_calc.cost_slvg_chp_2023()
                cost_salvage_hp = salvage_calc.cost_slvg_hp_2023()
                cost_salvage_p2g = salvage_calc.cost_slvg_p2g_2023()
                ...
                f1_hp = (cost_inv_hp / ((1 + 0.05) ** (idx_w - 1))) + (cost_om_hp / ((1 + 0.05) ** y))
                f1_p2g = (cost_inv_p2g / ((1 + 0.05) ** (idx_w - 1))) + (cost_om_p2g / ((1 + 0.05) ** y))
                ...
                f1_slvg = cost_salvage_pv + cost_salvage_bess + cost_salvage_chp + cost_salvage_chp + cost_salvage_hp + cost_salvage_p2g

                f1 = (f1_pv + f1_bess + f1_chp + f1_hp + f1_p2g + f1_p_import + f1_gas_import) - (f1_p_export + f1_slvg)
                ...
                res_heat_produced_CHP_mwh_2024.append(h_chp_mwh)
                print("heat_produced_CHP_mwh_2024 =", h_chp_mwh)
                ...
                power_flow_ = power_flow_calc(num_time_step=1, net=net, x_pv_size=x[45:60], x_pv_bus=x[30:45],
                                              pv_data=pv_data, bess_bus=x[309], bess_mw=x[272],
                ...
                if p_surplus_mw >= p_p2g_mw:
                    ...
                    if h2_MWh_day > g_demand_mwh_2024:
                        h2_mwh_balance = np.absolute(h2_MWh_day - g_demand_mwh_2024)
                    ...
                    if p_surplus_4_hp_bess_mw >= x_hp:
                        ...
                        if heat_production_hp_mwh > 0:
                            heat_production_hp_mwh = heat_production_hp_mwh * 24
                            print("heat_production_hp_mwh_2024 = ", heat_production_hp_mwh)
                        ...
                        if p_surplus_after_hp_mw >= x_bess:
                            add_bess = power_flow_.create_bess_init()
                            vm = power_flow_.calc_vm()
                            res_bess_2024_mwh.append(x_bess * 4.5)  # charging with max sun hour in Germany
                            res_p_curt_2024_mwh.append(((p_surplus_after_hp_mw - x_bess) * 24))
                        else:
                            # ***************** Energy export ***************************
                            res_p_curt_2024_mwh.append((p_surplus_after_hp_mw * 24))

                    # .... if p surplus is lower thaN x_HP.........................
                    else:
                        # ******************** BESS function ************************************************
                        if p_surplus_4_hp_bess_mw >= x_bess:
                            add_bess = power_flow_.create_bess_init()
                            ...
                            p_surplus_after_bess_mw = p_surplus_4_hp_bess_mw - x_bess
                            if p_surplus_after_bess_mw > 0:
                                res_p_curt_2024_mwh.append(p_surplus_after_bess_mw * 24)
                        else:
                            # ***************** Energy export ***************************
                            res_p_curt_2024_mwh.append((p_surplus_4_hp_bess_mw * 24))
                    ...
                    if h2_MWh_day > g_demand_mwh_2024:
                        h2_mwh_balance = (h2_MWh_day - g_demand_mwh_2024)
                        res_gas_storage_2024_mwh.append(h2_mwh_balance)
                        gas_import = 0
                        res_gas_import_4ind_2024_mwh.append(gas_import)
                    else:
                        gas_import = (g_demand_mwh_2024 - h2_MWh_day)
                        res_gas_import_4ind_2024_mwh.append(gas_import)
                        res_gas_storage_2024_mwh.append(0)
                ...
                # Interconnection of HP and CHP
                if res_hp_2024_mwh[-1] >= heat[1]:
                    res_heat_produced_CHP_mwh_2024[-1] = 0
                    # res_heat_produced_CHP_mwh_2023.append(0)
                    h_surplus = res_hp_2024_mwh[-1] - heat[1]

                    # HEAT STORAGE ************************
                    res_heat_storage_2024.append(h_surplus)
                else:
                    res_hp_2024_mwh[-1] = 0

                    # HEAT STORAGE ************************
                    res_heat_storage_2024.append(0)

                print("res_heat_produced_CHP_mwh_2024", res_heat_produced_CHP_mwh_2024)
                print("res_hp_2024_mwh", res_hp_2024_mwh)
                print("res_heat_storage_2024", res_heat_storage_2024)
                ...
                cost_om_hp = cost.cost_om_2024_hp()

                cost_inv_p2g = cost.cost_inv_2024_p2g()
                cost_om_p2g = cost.cost_om_2024_p2g()
                ...
                f2_hp = (cost_inv_hp / ((1 + 0.05) ** (idx_w - 1))) + (cost_om_hp / ((1 + 0.05) ** y))
                f2_p2g = (cost_inv_p2g / ((1 + 0.05) ** (idx_w - 1))) + (cost_om_p2g / ((1 + 0.05) ** y))
                # f2_p_exp_mwh =
                f2_p_import = sum(res_p_grid_import_2024_mwh) * (4.35 + 32)  # (average typical day ahead + market premium)
                f2_p_export = sum(res_p_curt_2024_mwh) * 72
                f2_gas_import = (sum(res_gas_import_4ind_2024_mwh) + sum(res_gas_import_4chp_2024_mwh)) * 210  # ../MWh
                f1_slvg = cost_salvage_pv + cost_salvage_bess + cost_salvage_chp + cost_salvage_chp + cost_salvage_hp + cost_salvage_p2g

                f2 = (f2_pv + f2_bess + f2_chp + f2_hp + f2_p2g + f2_p_import + f2_gas_import) - (f2_p_export + f1_slvg)
                ...
                res_heat_produced_CHP_mwh_2025.append(h_chp_mwh)
                print("heat_produced_CHP_mwh_2025 =", h_chp_mwh)
                ...
                power_flow_ = power_flow_calc(num_time_step=1, net=net, x_pv_size=x[75:90], x_pv_bus=x[60:75],
                                              pv_data=pv_data, bess_bus=x[310], bess_mw=x[273],
                ...
                # ..... Power balance: ................................................
                p_balance_mwh = power_flow_.calc_p_balance()
                # print("p_balance_2025_mwh =", p_balance_mwh)
                ...
                if p_surplus_mw >= p_p2g_mw:
                    ...
                    if h2_MWh_day > g_demand_mwh_2025:
                        h2_mwh_balance = np.absolute(h2_MWh_day - g_demand_mwh_2025)
                    ...
                    if p_surplus_4_hp_bess_mw >= x_hp:
                        ...
                        if p_surplus_after_hp_mw >= x_bess:
                            add_bess = power_flow_.create_bess_init()
                            vm = power_flow_.calc_vm()
                            res_bess_2025_mwh.append(x_bess * 4.5)  # charging with max sun hour in Germany
                            res_p_curt_2025_mwh.append(((p_surplus_after_hp_mw-x_bess) * 24))
                        else:
                            # ***************** Energy export ***************************
                            res_p_curt_2025_mwh.append((p_surplus_after_hp_mw * 24))
                    ...
                    # .... if p surplus is lower thaN x_HP.........................
                        ...
                        if p_surplus_4_hp_bess_mw >= x_bess:
                            ...
                            p_surplus_after_bess_mw = p_surplus_4_hp_bess_mw - x_bess
                            if p_surplus_after_bess_mw > 0:
                                res_p_curt_2025_mwh.append(p_surplus_after_bess_mw * 24)
                        else:
                            # ***************** Energy export ***************************
                            res_p_curt_2025_mwh.append((p_surplus_4_hp_bess_mw * 24))
                else:
                    p_p2g_mw = p_surplus_mw
                    h2_production = p2g_func(p_input_mw=p_p2g_mw)
                    h2_MWh_day = h2_production.p2g()
                    ...
                    # ------- Outputs -------------------
                    res_h2_2025_mwh.append(h2_MWh_day)
                    res_hp_2025_mwh.append(0)
                    res_bess_2025_mwh.append(0)
                    res_p_curt_2025_mwh.append(0)

                # Interconnection of HP and CHP
                if res_hp_2025_mwh[-1] >= heat[1]:
                    res_heat_produced_CHP_mwh_2025[-1] = 0
                    # res_heat_produced_CHP_mwh_2023.append(0)
                    h_surplus = res_hp_2025_mwh[-1] - heat[1]
                    # HEAT STORAGE ************************
                    res_heat_storage_2025.append(h_surplus)
                else:
                    res_hp_2025_mwh[-1] = 0
                    # HEAT STORAGE ************************
                    res_heat_storage_2025.append(0)

                print("res_heat_produced_CHP_mwh_2025", res_heat_produced_CHP_mwh_2025)
                print("res_hp_2025_mwh", res_hp_2025_mwh)
                print("res_heat_storage_2025", res_heat_storage_2025)
                ...
                cost_om_hp = cost.cost_om_2025_hp()

                cost_inv_p2g = cost.cost_inv_2025_p2g()
                cost_om_p2g = cost.cost_om_2025_p2g()
                ...
                f3_hp = (cost_inv_hp / ((1 + 0.05) ** (idx_w - 1))) + (cost_om_hp / ((1 + 0.05) ** y))
                f3_p2g = (cost_inv_p2g / ((1 + 0.05) ** (idx_w - 1))) + (cost_om_p2g / ((1 + 0.05) ** y))
                # f2_p_exp_mwh =
                f3_p_import = sum(res_p_grid_import_2025_mwh) * (4.35 + 32)  # (average typical day ahead + market premium)
                f3_p_export = sum(res_p_curt_2025_mwh) * 72
                f3_gas_import = (sum(res_gas_import_4ind_2025_mwh) + sum(res_gas_import_4chp_2025_mwh)) * 250  # ../MWh
                f1_slvg = cost_salvage_pv + cost_salvage_bess + cost_salvage_chp + cost_salvage_chp + cost_salvage_hp + cost_salvage_p2g

                f3 = (f3_pv + f3_bess + f3_chp + f3_hp + f3_p2g + f3_p_import + f3_gas_import) - (f3_p_export + f1_slvg)
                ...
                res_heat_produced_CHP_mwh_2026.append(h_chp_mwh)
                print("heat_produced_CHP_mwh_2026 =", h_chp_mwh)
                ...
                power_flow_ = power_flow_calc(num_time_step=1, net=net, x_pv_size=x[105:120], x_pv_bus=x[90:105],
                                              pv_data=pv_data, bess_bus=x[311], bess_mw=x[274],
                ...
                if p_surplus_mw >= p_p2g_mw:
                    ...
                    if h2_MWh_day > g_demand_mwh_2026:
                        h2_mwh_balance = np.absolute(h2_MWh_day - g_demand_mwh_2026)
                    ...
                    if p_surplus_4_hp_bess_mw >= x_hp:
                        ...
                        if p_surplus_after_hp_mw >= x_bess:
                            add_bess = power_flow_.create_bess_init()
                            vm = power_flow_.calc_vm()
                            res_bess_2026_mwh.append(x_bess * 4.5)  # charging with max sun hour in Germany
                            res_p_curt_2026_mwh.append(((p_surplus_after_hp_mw - x_bess) * 24))
                        else:
                            # ***************** Energy export ***************************
                            res_p_curt_2026_mwh.append((p_surplus_after_hp_mw * 24))
                    ...
                    # .... if p surplus is lower thaN x_HP.........................
                        ...
                        if p_surplus_4_hp_bess_mw >= x_bess:
                            ...
                            p_surplus_after_bess_mw = p_surplus_4_hp_bess_mw - x_bess
                            if p_surplus_after_bess_mw > 0:
                                res_p_curt_2026_mwh.append(p_surplus_after_bess_mw * 24)
                        else:
                            # ***************** Energy export ***************************
                            res_p_curt_2026_mwh.append((p_surplus_4_hp_bess_mw * 24))
                else:
                    p_p2g_mw = p_surplus_mw
                    h2_production = p2g_func(p_input_mw=p_p2g_mw)
                    h2_MWh_day = h2_production.p2g()
                    ...
                    # ------- Outputs -------------------
                    res_h2_2026_mwh.append(h2_MWh_day)
                    res_hp_2026_mwh.append(0)
                    res_bess_2026_mwh.append(0)
                    res_p_curt_2026_mwh.append(0)

                # Interconnection of HP and CHP
                if res_hp_2026_mwh[-1] >= heat[1]:
                    res_heat_produced_CHP_mwh_2026[-1] = 0
                    # res_heat_produced_CHP_mwh_2023.append(0)
                    h_surplus = res_hp_2026_mwh[-1] - heat[1]
                    # HEAT STORAGE ************************
                    res_heat_storage_2026.append(h_surplus)
                else:
                    res_hp_2025_mwh[-1] = 0
                    # HEAT STORAGE ************************
                    res_heat_storage_2026.append(0)

                print("res_heat_produced_CHP_mwh_2026", res_heat_produced_CHP_mwh_2026)
                print("res_hp_2026_mwh", res_hp_2026_mwh)
                print("res_heat_storage_2026", res_heat_storage_2026)
                ...
                cost_om_hp = cost.cost_om_2026_hp()

                cost_inv_p2g = cost.cost_inv_2026_p2g()
                cost_om_p2g = cost.cost_om_2026_p2g()
                ...
                f4_hp = (cost_inv_hp / ((1 + 0.05) ** (idx_w - 1))) + (cost_om_hp / ((1 + 0.05) ** y))
                f4_p2g = (cost_inv_p2g / ((1 + 0.05) ** (idx_w - 1))) + (cost_om_p2g / ((1 + 0.05) ** y))
                # f2_p_exp_mwh =
                ...
                f4_gas_import = (sum(res_gas_import_4ind_2026_mwh) + sum(res_gas_import_4chp_2026_mwh)) * 345  # ../MWh
                f1_slvg = cost_salvage_pv + cost_salvage_bess + cost_salvage_chp + cost_salvage_chp + cost_salvage_hp + cost_salvage_p2g

                f4 = (f4_pv + f4_bess + f4_chp + f4_hp + f4_p2g + f4_p_import + f4_gas_import) - (f4_p_export + f1_slvg)
                ...
                res_heat_produced_CHP_mwh_2027.append(h_chp_mwh)
                print("heat_produced_CHP_mwh_2027 =", h_chp_mwh)
                ...
                power_flow_ = power_flow_calc(num_time_step=1, net=net, x_pv_size=x[135:150], x_pv_bus=x[120:135],
                                              pv_data=pv_data, bess_bus=x[312], bess_mw=x[275],
                ...
                if p_surplus_mw >= p_p2g_mw:
                    ...
                    if h2_MWh_day > g_demand_mwh_2027:
                        h2_mwh_balance = np.absolute(h2_MWh_day - g_demand_mwh_2027)
                    ...
                    if p_surplus_4_hp_bess_mw >= x_hp:
                        ...
                        if p_surplus_after_hp_mw >= x_bess:
                            add_bess = power_flow_.create_bess_init()
                            vm = power_flow_.calc_vm()
                            res_bess_2027_mwh.append(x_bess * 4.5)  # charging with max sun hour in Germany
                            res_p_curt_2027_mwh.append(((p_surplus_after_hp_mw - x_bess) * 24))
                        else:
                            # ***************** Energy export ***************************
                            res_p_curt_2027_mwh.append((p_surplus_after_hp_mw * 24))
                    ...
                    # .... if p surplus is lower thaN x_HP.........................
                        ...
                        if p_surplus_4_hp_bess_mw >= x_bess:
                            ...
                            p_surplus_after_bess_mw = p_surplus_4_hp_bess_mw - x_bess
                            if p_surplus_after_bess_mw > 0:
                                res_p_curt_2027_mwh.append(p_surplus_after_bess_mw * 24)
                        else:
                            # ***************** Energy export ***************************
                            res_p_curt_2027_mwh.append((p_surplus_4_hp_bess_mw * 24))
                else:
                    p_p2g_mw = p_surplus_mw
                    h2_production = p2g_func(p_input_mw=p_p2g_mw)
                    h2_MWh_day = h2_production.p2g()
                    ...
                    # ------- Outputs -------------------
                    res_h2_2027_mwh.append(h2_MWh_day)
                    res_hp_2027_mwh.append(0)
                    res_bess_2027_mwh.append(0)
                    res_p_curt_2027_mwh.append(0)

                # Interconnection of HP and CHP
                if res_hp_2027_mwh[-1] >= heat[1]:
                    res_heat_produced_CHP_mwh_2027[-1] = 0
                    # res_heat_produced_CHP_mwh_2023.append(0)
                    h_surplus = res_hp_2027_mwh[-1] - heat[1]
                    # HEAT STORAGE ************************
                    res_heat_storage_2027.append(h_surplus)
                else:
                    res_hp_2025_mwh[-1] = 0
                    # HEAT STORAGE ************************
                    res_heat_storage_2027.append(0)

                print("res_heat_produced_CHP_mwh_2026", res_heat_produced_CHP_mwh_2027)
                print("res_hp_2026_mwh", res_hp_2027_mwh)
                print("res_heat_storage_2026", res_heat_storage_2027)
                ...
                cost_om_hp = cost.cost_om_2027_hp()

                cost_inv_p2g = cost.cost_inv_2027_p2g()
                cost_om_p2g = cost.cost_om_2027_p2g()
                ...
                f5_hp = (cost_inv_hp / ((1 + 0.05) ** (idx_w - 1))) + (cost_om_hp / ((1 + 0.05) ** y))
                f5_p2g = (cost_inv_p2g / ((1 + 0.05) ** (idx_w - 1))) + (cost_om_p2g / ((1 + 0.05) ** y))
                # f2_p_exp_mwh =
                f5_p_import = sum(res_p_grid_import_2027_mwh) * (4.35 + 32)  # (average typical day ahead + market premium)
                f5_p_export = sum(res_p_curt_2027_mwh) * 72
                f5_gas_import = (sum(res_gas_import_4ind_2027_mwh) + sum(res_gas_import_4chp_2027_mwh)) * 345  # ../MWh
                f1_slvg = cost_salvage_pv + cost_salvage_bess + cost_salvage_chp + cost_salvage_chp + cost_salvage_hp + cost_salvage_p2g

                f5 = (f5_pv + f5_bess + f5_chp + f5_hp + f5_p2g + f5_p_import + f5_gas_import) - (f5_p_export + f1_slvg)
                ...
                res_heat_produced_CHP_mwh_2028.append(h_chp_mwh)
                print("heat_produced_CHP_mwh_2028 =", h_chp_mwh)
                ...
                power_flow_ = power_flow_calc(num_time_step=1, net=net, x_pv_size=x[165:180], x_pv_bus=x[150:165],
                                              pv_data=pv_data, bess_bus=x[313], bess_mw=x[276],
                ...
                if p_surplus_mw >= p_p2g_mw:
                    ...
                    if h2_MWh_day > g_demand_mwh_2028:
                        h2_mwh_balance = np.absolute(h2_MWh_day - g_demand_mwh_2028)
                    ...
                    # .... if p surplus is lower thaN x_HP.........................
                        ...
                        if p_surplus_4_hp_bess_mw >= x_bess:
                            add_bess = power_flow_.create_bess_init()
                            vm = power_flow_.calc_vm()
                            res_bess_2028_mwh.append(x_bess * 4.5)  # charging with max sun hour in Germany

                            p_surplus_after_bess_mw = p_surplus_4_hp_bess_mw - x_bess
                            if p_surplus_after_bess_mw > 0:
                                res_p_curt_2028_mwh.append(p_surplus_after_bess_mw * 24)
                        else:
                            # ***************** Energy export ***************************
                            res_p_curt_2028_mwh.append((p_surplus_4_hp_bess_mw * 24))
                else:
                    p_p2g_mw = p_surplus_mw
                    h2_production = p2g_func(p_input_mw=p_p2g_mw)
                    h2_MWh_day = h2_production.p2g()
                    ...
                    # ------- Outputs -------------------
                    res_h2_2028_mwh.append(h2_MWh_day)
                    res_hp_2028_mwh.append(0)
                    res_bess_2028_mwh.append(0)
                    res_p_curt_2028_mwh.append(0)

                # Interconnection of HP and CHP
                if res_hp_2028_mwh[-1] >= heat[1]:
                    res_heat_produced_CHP_mwh_2028[-1] = 0
                    # res_heat_produced_CHP_mwh_2023.append(0)
                    h_surplus = res_hp_2028_mwh[-1] - heat[1]
                    # HEAT STORAGE ************************
                    res_heat_storage_2028.append(h_surplus)
                else:
                    res_hp_2025_mwh[-1] = 0
                    # HEAT STORAGE ************************
                    res_heat_storage_2028.append(0)

                print("res_heat_produced_CHP_mwh_2028", res_heat_produced_CHP_mwh_2028)
                print("res_hp_2028_mwh", res_hp_2028_mwh)
                print("res_heat_storage_2028", res_heat_storage_2028)
                ...
                cost_om_hp = cost.cost_om_2028_hp()

                cost_inv_p2g = cost.cost_inv_2028_p2g()
                cost_om_p2g = cost.cost_om_2028_p2g()
                ...
                f6_hp = (cost_inv_hp / ((1 + 0.05) ** (idx_w - 1))) + (cost_om_hp / ((1 + 0.05) ** y))
                f6_p2g = (cost_inv_p2g / ((1 + 0.05) ** (idx_w - 1))) + (cost_om_p2g / ((1 + 0.05) ** y))
                # f2_p_exp_mwh =
                f6_p_import = sum(res_p_grid_import_2028_mwh) * (4.35 + 32)  # (average typical day ahead + market premium)
                f6_p_export = sum(res_p_curt_2028_mwh) * 72
                f6_gas_import = (sum(res_gas_import_4ind_2028_mwh) + sum(res_gas_import_4chp_2028_mwh)) * 345  # ../MWh
                f1_slvg = cost_salvage_pv + cost_salvage_bess + cost_salvage_chp + cost_salvage_chp + cost_salvage_hp + cost_salvage_p2g

                f6 = (f6_pv + f6_bess + f6_chp + f6_hp + f6_p2g + f6_p_import + f6_gas_import) - (f6_p_export + f1_slvg)
        ...
        # out["F"] = [f1_pv, f1_bess, f1_chp, f1_hp, f1_p2g, f3]
        out["F"] = [f1_pv, f1_bess, f1_chp, f1_hp, f1_p2g,
                    f2_pv, f2_bess, f2_chp, f2_hp, f2_p2g,
                    f3_pv, f3_bess, f3_chp, f3_hp, f3_p2g,
                    f4_pv, f4_bess, f4_chp, f4_hp, f4_p2g,
                    f5_pv, f5_bess, f5_chp, f5_hp, f5_p2g,
                    f6_pv, f6_bess, f6_chp, f6_hp, f6_p2g]
...
res_h2_production = pd.DataFrame(list(zip(res_h2_2023_mwh, res_h2_2024_mwh,
                                 res_h2_2025_mwh, res_h2_2026_mwh,
                                 res_h2_2027_mwh, res_h2_2028_mwh)),
                        columns=['res_h2_2023_mwh', 'res_h2_2024_mwh',
                                 'res_h2_2025_mwh', 'res_h2_2026_mwh',
                                 'res_h2_2027_mwh', 'res_h2_2028_mwh'])
pd.DataFrame(res_h2_production.to_csv('results/res_h2_production.csv', index=False))


res_gas_import_4chp = pd.DataFrame(list(zip(res_gas_import_4chp_2023_mwh, res_gas_import_4chp_2024_mwh,
                                 res_gas_import_4chp_2025_mwh, res_gas_import_4chp_2026_mwh,
                                 res_gas_import_4chp_2027_mwh, res_gas_import_4chp_2028_mwh)),
                        columns=['res_gas_import_4chp_2023_mwh', 'res_gas_import_4chp_2024_mwh',
                                 'res_gas_import_4chp_2025_mwh', 'res_gas_import_4chp_2026_mwh',
                                 'res_gas_import_4chp_2027_mwh', 'res_gas_import_4chp_2028_mwh'])
pd.DataFrame(res_gas_import_4chp.to_csv('results/res_gas_import_4chp.csv', index=False))


res_heat_production = pd.DataFrame(list(zip(res_heat_produced_CHP_mwh_2023,
                                            res_heat_produced_CHP_mwh_2024,
                                            res_heat_produced_CHP_mwh_2025,
                                            res_heat_produced_CHP_mwh_2026,
                                            res_heat_produced_CHP_mwh_2027,
                                            res_heat_produced_CHP_mwh_2028,
                                            res_hp_2023_mwh, res_hp_2024_mwh,
                                            res_hp_2025_mwh, res_hp_2026_mwh,
                                            res_hp_2027_mwh, res_hp_2028_mwh)),
                                    columns=['res_heat_produced_CHP_mwh_2023',
                                            'res_heat_produced_CHP_mwh_2024',
                                            'res_heat_produced_CHP_mwh_2025',
                                            'res_heat_produced_CHP_mwh_2026',
                                            'res_heat_produced_CHP_mwh_2027',
                                            ...
                                            'res_hp_2025_mwh', 'res_hp_2026_mwh',
                                            'res_hp_2027_mwh', 'res_hp_2028_mwh'])
pd.DataFrame(res_heat_production.to_csv('results/res_heat_production.csv', index=False))
